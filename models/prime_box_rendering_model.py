"""
Implements a generative model of partially occluded cardboard box observation images:
...
class Model(nn.Module):
    def __init__(self, filename_obj, filename_ref=None):
        ...
        self.viewing_angle = nn.Parameter(torch.tensor(20.))

        # setup renderer
        renderer = nr.Renderer(camera_mode='look_at', viewing_angle=self.viewing_angle,
                               perspective=True)
...
def main():
    ...
    for i in loop:
        ...
        image = (images.detach().cpu().numpy()[0].transpose(1,2,0)*255).astype(np.uint8)
        imsave('/tmp/_tmp_%04d.png' % i, image)
        loop.set_description('Optimizing (loss %.4f)' % loss.data)
        print("Current viewing angle: ", model.viewing_angle.item())
        if loss.item() < 70:
            break
