...
def inversed_image_transform_homo(transform):
    # type: (np.ndarray) -> np.ndarray
    transform_homo = np.zeros((3, 3))
    transform_homo[0:2, :] = transform
    transform_homo[2, 2] = 1
    inversed_homo = np.linalg.inv(transform_homo)
    return inversed_homo
